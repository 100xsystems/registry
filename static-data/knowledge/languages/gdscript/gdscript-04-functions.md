---
{
  "title": "Functions",
  "description": "Define and call functions.",
  "type": "lesson",
  "order": 4,
  "duration": "45 min",
  "difficulty": "beginner",
  "learning_objectives": [
    "Write functions",
    "Pass arguments",
    "Return values",
    "Use default params"
  ],
  "knowledge_refs": [
    "gdscript/gdscript-04-functions"
  ],
  "prerequisites": [
    "GDScript-03: Control Flow"
  ],
  "references": [
    {
      "title": "Godot Docs: GDScript",
      "url": "https://docs.godotengine.org/en/stable/tutorials/scripting/gdscript/index.html",
      "description": "Official documentation"
    },
    {
      "title": "GDScript Reference",
      "url": "https://docs.godotengine.org/en/stable/tutorials/scripting/gdscript/gdscript_basics.html",
      "description": "Language reference"
    },
    {
      "title": "Godot Community",
      "url": "https://godotengine.org/community/",
      "description": "Community links"
    }
  ]
}
---

# GDSCRIPT-04-FUNCTIONS: Functions

## Introduction

Define and call functions. By the end of this lesson you will be able to: Write functions; Pass arguments; Return values; Use default params.

## Key Concepts

### 1. Write functions

Target: Write functions. Start with the foundations — read the runnable example carefully and trace its output before moving on.

```gdscript
func add(a, b):
    return a + b

func _ready():
    print(add(2, 3))
```
### 2. Pass arguments

Target: Pass arguments. Apply the idiomatic pattern — this is how production code expresses this idea, so study the shape of the code.

```gdscript
func greet(name: String) -> String:
    return "Hello, " + name
```
### 3. Return values

Target: Return values. Watch for the edge cases — this is where subtle bugs hide, and experienced developers reason about them explicitly.

```gdscript
func move(dx := 0.0, dy := 0.0) -> void:
    position.x += dx
    position.y += dy
```
### 4. Use default params

Target: Use default params. Put it together — extend the example to combine this concept with what you learned in earlier lessons.

```gdscript
func is_even(n: int) -> bool:
    return n % 2 == 0
```

## Practice Questions

1. What is the key idea behind "Functions"?
1. Write a small program that exercises at least two concepts from this lesson.
1. How would you explain this topic to a fellow developer in one paragraph?

## LLM Prompts for Deeper Understanding

1. "Explain Functions with analogies and real-world examples"
1. "Show me common mistakes beginners make with Functions"
1. "Provide advanced patterns and performance considerations for Functions"

## Key Takeaways

- Master the core ideas of Functions through practice
- Combine this lesson with prior lessons to build real programs
- Explore the linked official documentation for authoritative depth

## Further Reading

Dive deeper into this topic using the reference resources listed in the frontmatter.
